...
export abstract class BaseDataService<T> extends ReadableDataServiceService<T> {
  ...
  update<TEntity = T, TResult = T>(entity: TEntity, isFormData: boolean = false): Observable<BaseResponse<TResult>> {
    return this.http.put<BaseResponse<TResult>>(this.serviceBaseUrl, isFormData == true ? FormDataHeplper.toFormData(entity) : entity);
  }
